...
<div class="container-fluid">
  <div class="row">
  <center>
    <form id="form1" name="form1" method="post" action="modificaraltaBD.php" onSubmit="return validacion()">
      <?php
include("conexion.inc");

$vf= $_POST['fi'];

$vSql = "SELECT*FROM movimientos_poliza where $vf=id_movimientopoliza";
      ...
      <p>
       ...
       <strong> <label for="textfield5">Estado:</label></strong>
        <select type="text" name="Estado" id="Estado" >
          <option value="<?php echo ($vResultado['estado']); ?>"><?php echo ($vResultado['estado']); ?></option>
          <option value="emitido">Emitido</option>
          <option value="pendiente">Pendiente</option>
        </select>
        <label for="Id">Id Prestación:</label>
        <input name="Id" type="text" id="Id" value="<?php echo ($vResultado['id_prestacion']); ?>">
        </p>
      <p>&nbsp;</p>
      <p>
        <label for="FechaV">Fecha vigencia:</label>
        <input name="FechaV" type="text" id="FechaV" value="<?php echo ($vResultado['fecha_vigencia']); ?>"  value="aaaa-mm-dd">
        <strong>
        Patente/s
          <label for="Patente">:</label>
        </strong>
<input name="Patente" type="text" id="Patente" value="<?php echo ($vResultado['patente']); ?>">
        <label for="Edificio">Edificio de riesgo:</label>
        <input name="Edificio" type="text" id="Edificio" value="<?php echo ($vResultado['edificio_riesgo']); ?>">
        </p>
      <p>&nbsp;</p>
      <p><strong>AP
         <label for="AP">:</label></strong>
        
       <input name="AP" type="text" id="AP" value="<?php echo ($vResultado['AP']); ?>">
      <strong>  <label for="Asegurado">Asegurado/s:</label></strong>
        <input name="Asegurado" type="text" id="Asegurado" value="<?php echo ($vResultado['asegurado']); ?>">
          <input  name="id_m" type="hidden" id="id_m" value="<?php echo ($vResultado['id_movimientopoliza']); ?>" >
      </p>
      <p>&nbsp;</p>
      <p>
        
        <input type="submit" href="modificaraltaBD.php" name="button" id="button" title="Salir" value="Registrar">
        <a href="../inicioUsuario.php">   <input type="button"  name="Salir" id="Salir" value="Salir">
        </A>        </p>
    </form>
    <p>&nbsp;</p>
  </center>
