...
<div class="container-fluid">
  <div class="row">
  <center>
    <form id="form1" name="form1" method="post" action="modificarcuentaBD.php" onSubmit="return validacion()">
     <?php
include("conexion.inc");

$vf= $_POST['fi'];

$vSql = "SELECT id_cuenta, apellido_cuenta, nombre_cuenta, tipo_usuario, email_usuario, usuario, clave FROM cuentas_usuarios where $vf=id_cuenta";
...
?>
      <p>
        ...
        <input name="Apellido" type="text" id="Apellido" value="<?php echo ($vResultado['apellido_cuenta']); ?>" >
        <strong>Nombre
          <label for="textfield2">:</label>
          </strong>
  <input name="Nombre" type="text" id="Nombre" value="<?php echo ($vResultado['nombre_cuenta']); ?>" >
        <strong>Tipo cuenta
          <label for="textfield3">:</label>
          </strong>
  <select type="text" name="Tipo" id="Tipo">
   <option value="<?php echo ($vResultado['tipo_usuario']); ?>"><?php echo ($vResultado['tipo_usuario']); ?></option>
    <option value="Usuario">Usuario</option>
    <option value="Administrador">Administrador</option>
    <option value="Persona Egreso">Persona Egreso</option>
   </select> <br><br>
        <label for="textfield4">Nombre Usuario:</label>
        <input name="Usuario" type="text" id="Usuario" value="<?php echo ($vResultado['usuario']); ?>">
        <label for="textfield5">Clave:</label>
        <input name="Clave" type="text" id="Clave" value="<?php echo ($vResultado['clave']); ?>">
        <label for="textfield">Email:</label>
        <input type="text" name="Email" id="Email" value="<?php echo ($vResultado['email_usuario']); ?>">
         <input  name="id_c" type="hidden" id="id_c" value="<?php echo ($vResultado['id_cuenta']); ?>" >
      </p>
      <p>&nbsp;</p>
      <p>
        <input type="submit" href="modificarcuentaBD.php"  name="Registrar" id="Registrar" value="Registrar">
